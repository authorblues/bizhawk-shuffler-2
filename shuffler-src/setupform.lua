...
local function create_plugin_settings_window(plugin, plist, px)
	...
	local SETTINGS_TYPES =
	{
		...
		['info'] = {
			make = function(setting, y)
				forms.label(form, setting.text, 10, y+3, 300, 20)
			end,
		},
	}
...
local function plugin_setup(plist, px)
...
local function save_new_settings()
	config = {}
